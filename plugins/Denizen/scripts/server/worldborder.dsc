...
border_cmd:
    ...
    usage: /border
    tab complete:
    - determine <list[select|save]>
    script:
    - inject permission_op
    - if !<list[select|save].contains[<context.args.get[1]||null>]>:
        - narrate <gray>-----------------------------
        - narrate "<yellow>/border select"
        - narrate "  <gray>- Setzt WE-selection auf die WG-region <white>spielwelt"
        - narrate "<yellow>/border save"
        - narrate "  <gray>- Setzt die WG-region und Grenze auf die aktuelle WE-selection"
        - narrate <gray>-----------------------------
        - stop
    - define reg <region[spielwelt,world]>
    - if <context.args.get[1]> == select:
        - execute as_player "rg select spielwelt"
        - narrate "<gray>Spielwelt markiert."
        - stop
    - else if <context.args.get[1]> == save:
        - execute as_player "rg update spielwelt"
        - note <player.we_selection> as:spielwelt
        - narrate "<gray>Region neu gesetzt."
        - narrate "<gray>Grenze neu gesetzt."
        - stop
    - note <player.we_selection> as:<[name]>
    - if !<context.args.get[2]||null> == null:
        - flag server grenzen.<[name]>:<map[warn/<context.args.get[2]>]>
...
grenze_world:
    ...
    events:
        ...
        - narrate "Leaving <context.area.note_name>"
        on player enters polygon:
        - narrate "Joining <context.area.note_name>"
